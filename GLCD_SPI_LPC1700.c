...
void GLCD_Bitmap (unsigned int x, unsigned int y, unsigned int w, unsigned int h, unsigned char *bitmap) {
  ...
  wr_cmd(0x22);
  wr_dat_start();
  for (i = (h-1)*w; i > -1; i -= w) {	//each for loop will write the pixel for one column, begenning from top right corner
    for (j = 0; j < w; j++) {
...
void GLCD_BitmapFull (unsigned char *bitmap) {
  int i, j;
	int w=160;
	int h=120;
  unsigned short *bitmap_ptr = (unsigned short *)bitmap;

  GLCD_SetWindow (0, 0, w*2, h*2);
...
void GLCD_BitmapZoom (unsigned int x_offset, unsigned int y_offset,unsigned int zoom_factor, unsigned char *bitmap)
{
  int i, j;
	int a,b;
	int h=120;		//this is the size of the original pictures we are displaying
	int w =160;
  unsigned short *bitmap_ptr = (unsigned short *)bitmap;

  GLCD_SetWindow (0, 0, 320, 240);	//we are using the LCD full capacity starting from the origin

  wr_cmd(0x22);
  wr_dat_start();
  for (i = (h-1)*w + (y_offset*w); i > -1 + (y_offset*w) ; i -= w) 
	{	//each for loop will write the pixel for one column, begenning from top right corner
		for (b=1; b<=zoom_factor;b++)		//we display the same row 4 times as well
		{
			for (j = x_offset; j < 2*w/(zoom_factor)+x_offset; j++)
				for (a=1; a<=zoom_factor;a++)
				 wr_dat_only (bitmap_ptr[i+j]);	//we display the same pixel 4 times
		}
	}
